...
public record SignalResponse(TickerSummary ticker, SignalCall call, String matchedRule, String ruleRationale,
                              String ruleTableVersion, IndicatorResponse indicators) {

    static SignalResponse of(IndicatorResponse indicators, SignalRuleId matchedRule) {
        return new SignalResponse(indicators.ticker(), matchedRule.call(), matchedRule.name(),
                matchedRule.rationale(), SignalRuleEngine.RULE_TABLE_VERSION, indicators);
    }
}
