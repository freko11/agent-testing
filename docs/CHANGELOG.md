...
**Story**: `HoldTermRule`'s own doc comment has carried the same flag since
E2-F3-S2 shipped it — "provisional engineering estimates, not yet
backtest-validated — revisit once E2-F4-S1's backtest harness exists and
can check realized hold-term accuracy, not just call win/loss." That
harness has existed since E2-F4-S1 and grew TP/SL-aware scoring
(E8-F2-S1), transaction costs (E8-F2-S2), and per-checkpoint holding-
duration tracking (E8-F2-S3) — `BacktestHarness.run` already scores every
BUY/SELL call at exactly its matched branch's MIN/MID/MAX day count — but
nothing had ever swept alternative day counts against a branch's own
decision-point population to check whether its provisional range is
actually good. Found during a review of E8's still-open flagged items
(the same kind of review that produced E8-F1-S8..S12/E8-F3-S6/E8-F5-S2);
filed as a new backlog feature, F8.6, since it doesn't fit any existing
F8.1-F8.5 grouping (see `docs/agile-plan.md`'s new E8-F6-S1 row).

### Design gate

Ran through the `Plan` agent first, per this repo's standing convention
for anything non-trivial. Key finding from that pass: no changes to
`BacktestHarness`/`HoldTermCalculator`/`HoldTermRule` were needed at all —
`BacktestReport.buySellDecisionPoints()` already carries everything a
calibration test needs to reclassify and rescore every historical
decision point (matched rule → `TrendStrength`, ATR% volatility →
`VolatilityBand` via `HoldTermCalculator`'s own public
`VOLATILITY_LOW_MAX`/`VOLATILITY_MEDIUM_MAX` constants, `HoldTermRule
.match` for the real branch resolver, decision index → decision
close/forward candles from the original candle list). This story is
therefore a single new test file,
`backend/src/test/java/.../backtest/HoldTermRangeCalibrationTest.java` —
no production surface added, unlike every E8-F1 threshold story (which
each added a new `RuleThresholds` field before it could even be swept).

### Mechanism

Each of BTCUSDT/DOGEUSDT/SOLUSDT's decision points is generated by
replaying `BacktestHarness.run` through the *exact* production gate chain
`SignalService.computeSignalWithProvenance` applies (per-symbol
thresholds → SELL regime gate → SELL MA-crossover gate, all crypto-only),
so the classified population matches what real hold-term suggestions are
generated for today, not a bare-default replay. Every BUY/SELL point is
then classified into one of the 6 `HoldTermRule` branches and scored at a
13-point day grid (`{1,2,3,4,5,6,7,8,10,12,15,20,25}`, spanning below the
narrowest current branch through beyond the widest) via
`WalkForwardScorer` — each point's TP/SL crossing scan runs once at the
grid's max day (25) and is reused across every candidate day, per
`findFirstCrossing`'s own "runs once, applied per checkpoint" contract,
avoiding N-times rescanning.

**Classification gotcha caught before it became a bug**: `STRONG_MEDIUM`
(3-10 days) and `MODERATE_LOW` (3-10 days) have byte-identical
`(minDays, maxDays)` — classifying from a decision point's derived
`HoldTerm.label()` alone would have silently merged two branches.
Classification goes through `TrendStrength`+`VolatilityBand` exclusively,
never through the derived `HoldTerm` value.

**Metric**: `expectancyPctAfterCosts()`, not
`expectancyPctAfterCostsAndFunding()` — a deliberate departure from
several recent E8 stories that print both. `HoldTermRule`/
`HoldTermCalculator` carry no asset-type parameter and are meant to
eventually generalize to stock hold-term suggestions (E8-F1-S7 already
added a stock fixture elsewhere in E8), but Binance Futures funding-rate
carry cost is broker/asset-mechanics-specific and doesn't apply to a
stock position at all — gating a signal-quality decision on it would bake
a cost that doesn't apply to every asset type this table serves into the
ship bar. Funding-adjusted figures aren't printed in this test at all,
not just excluded from the bar.

**New floor introduced**: `MIN_SCORED_FLOOR = 30`. No prior E8-F1/F3
calibration test hardcodes a minimum sample size — this axis partitions
an already BUY/SELL-only population *jointly* by `TrendStrength` AND
`VolatilityBand` at once (six-way), a structurally worse sparsity risk
than any single-axis E8-F1 threshold sweep. Candidates below the floor
are flagged inline (`[below floor]`) rather than silently trusted.

### Findings

**The sparsity risk flagged going in was worse than anticipated — it
wasn't a sparsity problem, it was near-total absence.** Across all three
fixtures' tuning windows *and* held-out tails, `STRONG_LOW`,
`STRONG_MEDIUM`, `STRONG_HIGH`, and `MODERATE_LOW` all showed **zero**
decision points, everywhere, every time (n=0 at every single day in the
grid, for every symbol, in both windows). Two structural reasons, visible
once the pooled/per-symbol curves are laid side by side:

1. `BULLISH_UNANIMOUS`/`BEARISH_UNANIMOUS` (the only two `SignalRuleId`s
   that map to `TrendStrength.STRONG`) essentially never match across
   ~2,100 combined decision points in these three real crypto fixtures —
   every directional call in this dataset resolves as a 2-of-3 majority,
   never a 3-of-3 unanimous vote. This makes all three `STRONG_*`
   branches dead code in practice for crypto, not just under-sampled.
2. `VolatilityBand.LOW` (ATR% < `HoldTermCalculator.VOLATILITY_LOW_MAX`,
   2.0%) essentially never occurs for these three crypto pairs either —
   consistent with crypto's generally higher baseline volatility than the
   2.0% cutoff assumes. This kills `MODERATE_LOW` for the same reason it
   kills `STRONG_LOW`, independent of the unanimous-vote issue.

Only `MODERATE_MEDIUM` and `MODERATE_HIGH` had real data to evaluate:

- **`MODERATE_MEDIUM`** (current 2-7 days): pooled tuning showed a
  monotonic increase peaking around d=15 (+0.614% vs. the current max's
  +0.402%), which looked like a case for widening — until the per-symbol
  held-out breakdown showed it was driven almost entirely by DOGEUSDT
  (whose own curve plateaus at +3.434% from d=7 onward, already fully
  captured by the current max). BTCUSDT's held-out curve moves the
  *opposite* direction — it peaks at d=4 (+0.138%) and actively
  *degrades* by d=15 (-0.137%) — and SOLUSDT is negative at every day in
  this branch, worst at longer days. Any range change chasing the pooled
  tuning signal would have regressed BTCUSDT materially. Textbook
  asset-divergent finding, same shape E8-F1-S1's threshold axes hit
  before their own per-symbol forks — no-ship, current 2-7 days retained.
- **`MODERATE_HIGH`** (current 1-4 days): pooled tuning was flat/noisy
  (every candidate within ~0.1pp of each other, no candidate materially
  beating d=4's -0.286%) — too thin a signal to call a winner. SOLUSDT's
  held-out curve (n=50, well above the floor) peaks almost exactly at the
  current max (d=4, +0.542%, the single best value in its whole curve)
  and declines afterward. BTCUSDT's held-out curve (n=14) looked like a
  strong case for widening but sits below `MIN_SCORED_FLOOR` and was
  excluded from the decision per this story's own floor rule. No-ship —
  the current range already sits at or near the best-evidenced point,
  and the only story for widening comes from a sample too thin to trust.

**Secondary finding, flagged but not acted on** (out of scope per this
story's own AC and the design gate's explicit scope boundary): with 4 of
6 branches never firing at all in real crypto data, whether
`BULLISH_UNANIMOUS`/`BEARISH_UNANIMOUS` should exist as a distinct
`TrendStrength.STRONG` tier at all — or whether the 2.0% `VOLATILITY_LOW_MAX`
cutoff is calibrated for an asset class crypto doesn't resemble — is an
open question this story's AC explicitly put out of scope ("changing
which `(TrendStrength, VolatilityBand)` pairs exist... a different,
not-yet-chartered axis"). Left for a future story if it's ever picked up.

### Outcome

No branch's `(minDays, maxDays)` changed.
`HoldTermCalculator.HOLD_TERM_TABLE_VERSION` stays `"v1"` — the
versioning scheme's whole reason for existing independently of
`SignalRuleEngine.RULE_TABLE_VERSION` (a hold-term revision shouldn't
force reinterpreting historical BUY/SELL calls) means an unchanged table
correctly stays unversioned-forward. `SignalRuleEngine.RULE_TABLE_VERSION`
was never touched — this story only ever read decision points that
production already resolved.

Docker wasn't available in this sandboxed session (same recurring
blocker prior E8 stories hit); no production code changed, so no
live-browser verification was needed beyond
`HoldTermRangeCalibrationTest`'s own real-fixture-backed run.
...
## Post-E8 follow-up — ran E8-F5-S3's shadow-scoring diagnostic against accumulated live data

### Context

E8-F5-S3 shipped with zero live `SignalCallEntry` history to sanity-check
against, so its own changelog entry ended with "no recommendation beyond
run this once live data accumulates." A user request to check whether
enough live data now exists prompted actually doing so: `docker compose
up -d` (Oracle XE — the bind-mounted `./oracle-data` directory turned out
to already hold data from a prior local run, not a fresh DB), then the
backend (`local` profile, env vars from `.env`). Registering BTCUSDT/
DOGEUSDT/SOLUSDT against `POST /api/tickers` returned `200 OK` (already
existed, `createdAt` late July 2026) rather than `201 Created`, confirming
this was pre-existing accumulated state, not data generated by this
session.

### Findings

`GET /api/monitoring/weighted-vote-shadow` (default 90-day lookback, also
checked at 365 days — identical result, so all of it is recent): 1,237
`SignalCallEntry` rows considered, 0 skipped, **1,237 in `agreeCount`, 0
in every disagreement bucket** (`weightedOnlyBuy`, `weightedOnlySell`,
`downgradedByWeighted` all zero). `WeightedVoteRuleEngine` — at its
current `IndicatorWeights.DEFAULT` calibration (MACD 0.714, RSI/
MA-crossover both 0.000, from E8-F3-S5) — has never once diverged from
`SignalRuleEngine`'s live 2-of-3 vote across every real signal call this
app has persisted. (`GET /api/monitoring/signal-drift` was checked too,
for context: only 2 `OrderAuditEntry` rows exist at all, both SELL, both
`rule_table_version=v1` — the app has accumulated far more raw signal
snapshots than actual filled orders, which tracks given this is a
personal paper-trading tool.)

This isn't a bug in the diagnostic or the engine — it's consistent with
E8-F3-S6's own finding that with only MACD nonzero-weighted, the
weighted engine's behavior collapses to a small number of regimes, and
apparently the live data never lands outside the one that ties the
unweighted vote byte-for-byte.

### Recommendation

No evidence either for or against wiring `WeightedVoteRuleEngine` into
production: swapping it in today would be a behavioral no-op given 100%
agreement on all observed live data, so there's nothing to gain by
switching and nothing at risk either. Leaving it unwired, per this
story's own scope (a wiring decision was always out of scope for E8-F5-
S3). Revisiting only makes sense if `IndicatorWeights` calibration
changes enough to actually produce divergence worth evaluating — this
result gives future calibration work a concrete, non-hypothetical target
("does this weighting choice ever disagree with the live vote, and is
the disagreement good") rather than only the two static backtest
fixtures E8-F3-S1/S5 tuned against.

### Verification

Read live via the running app, not inferred: `docker compose ps` showed
the Oracle XE healthcheck passing before starting the backend; `GET
/health` returned 200 before any API calls; login went through Spring
Security's real `formLogin` flow (`POST /api/auth/login`, form-encoded,
CSRF cookie primed via `GET /api/auth/csrf` then re-primed via `GET
